...
void InputStuInfo(void)
{
	...
	//ÕâÀï»¹ÒªÌí¼ÓÒ»¸ö¼ÆËã×Ü³É¼¨µÄº¯ÊýÄ£¿é  void CalcuScore(void); //¼ÆËã¹¦ÄÜ ×Ü³É¼¨ÆÀ·Ö
...
void ModifyStuInfo(void)
{
	...
	while (p != NULL)
	{
		if (strcmp(p->StuInfo.StuID, sfStuID) == 0) {
			printf("ÒÑÕÒµ½¸ÃÑ§Éú£¬ÔÙ´ÎÈ·ÈÏÊÇ·ñÐèÒªÐÞ¸Ä£¿£¨ÊäÈë1È·ÈÏÐÞ¸Ä£©");
			char ch = _getch();
			if (ch != 1) { return; } //ÕâÀïÐ´return»¹ÊÇbreakÄÄ¸ö¸üºÃÄØ£¿
			printf("ÇëÊäÈëÒªÐÞ¸ÄµÄÐÅÏ¢ Ñ§Ï°³É¼¨¼¨µã ÂÛÎÄÊýÁ¿ Éç»á¹¤×÷£¨¿Õ¸ñ¸ô¿ª£©£º\n");
